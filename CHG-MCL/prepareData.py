...
def prepare_data(opt):
    ...
    dd_data = pd.read_csv(opt.data_path + '/integrated_dise_sim_integrated_dise_sim_1_0_.csv', header=None) #疾病100
    dd_mat = np.array(dd_data)

    mm_data = pd.read_csv(opt.data_path + '/integrated_circ_sim_integrated_circ_sim_1_0_.csv', header=None) #circ561
    mm_mat = np.array(mm_data)

    mi_dis_data = pd.read_csv(opt.data_path + '/Association Matrix.csv', header=None)
    print(f"Association Matrixs的维度是: {mi_dis_data.shape}")
